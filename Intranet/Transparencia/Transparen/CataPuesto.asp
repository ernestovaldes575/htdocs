...
{ InstReal = "opener.top.PideDato.C04"+ConsSePu+".value= '"+ClavCata+"' "
  eval(InstReal)
  InstReal = "opener.top.PideDato.D04"+ConsSePu+".value= '"+DescCata+"' "
  eval(InstReal)
  window.close();
}
</script>
...
if (Request.Form+"." != "undefined.")
 {  //Cerrar Base
	...
	Response.Redirect("CataPuesto.asp?ParamB="+CampBusq) 
 }			  
else
 { BandQuer = false
   ...
   if (String(Request.QueryString("ParamB")) != "undefined" && 
	   ...
	   { CampBusq = Request.QueryString("ParamB")
	     CondBusq   += "AND CPUDescri like '%"+CampBusq+"%' "  }
