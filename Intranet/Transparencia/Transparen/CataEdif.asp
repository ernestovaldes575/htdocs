...
{ InstReal = "opener.top.PideDato.C02"+ConsSePu+".value= '"+ClavCata+"' "
  eval(InstReal)
  InstReal = "opener.top.PideDato.D02"+ConsSePu+".value= '"+DescCata+"' "
  eval(InstReal)
  window.close();
}
</script>
 ...
 { BandQuer = false
   ...
   if (String(Request.QueryString("ParamB")) != "undefined" && 
	   ...
	   { CampBusq = Request.QueryString("ParamB")
	     CondBusq   += "AND  CEDDescri like '%"+CampBusq+"%' "  }
	   
    //Datos del Servidor Publico , , 
	EjecInst  = "SELECT CEDClave AS Clave , CEDDescri AS Descri "+
				"FROM   TCEdificio "+
				"WHERE  CEDAyuntamiento = '"+ClavAyun+"' "+
						CondBusq +" "
				"ORDER BY CEDClave "
...
<table width="440" align="center" class="ListInfo">
  ...
  <tr> 
    ...
    <th width="311">Entidad Federativa </th>
    <th>&nbsp;</th>
  </tr>
  <% //Seleccionada
     while ( !CataCons.EOF  )
